...
export default function RequestBlood() {
  ...
  const [activeTab, setActiveTab] = useState<'list' | 'create' | 'received'>(
    'list',
  );
  ...
  useEffect(() => {
    if (activeTab === 'list') {
        ...
        try {
          ...
          if (response.ok) {
            ...
            const filtered = data.filter(
              (r: any) => r.status === 'Active' || r.status === 'Accepted',
            );
            setRequests(filtered);
          } else {
            console.error(
              'Failed to fetch requests, server responded:',
              response.status,
            );
          }
        } catch (error) {
          console.error(
            'Failed to fetch requests (Network error or server down):',
            error,
          );
        } finally {
      ...
      const fetchDirectRequests = async () => {
        ...
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_URL || ''}/api/direct-requests?type=received`,
            {
              headers: {
          ...
          if (response.ok) {
            ...
            console.error(
              'Failed to fetch direct requests, server responded:',
              response.status,
            );
          }
        } catch (error) {
    ...
    if (tab === 'received') {
      ...
      setTimeout(() => {
        if (requestRefs.current[highlight]) {
          requestRefs.current[highlight]?.scrollIntoView({
            behavior: 'smooth',
            block: 'center',
          });
        }
  ...
  const handleRespondRequest = async (id: string) => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || ''}/api/requests/${id}/respond`,
        {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        toast.success(
          'You have successfully offered to donate! The requester will be notified.',
        );
  ...
  const handleReviewResponse = async (
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || ''}/api/requests/${requestId}/responses/${donorId}/status`,
        {
          method: 'PUT',
          headers: {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          'Blood request submitted successfully! Donors have been notified.',
        );
  ...
  return (
    <div className="pt-20 min-h-screen bg-gray-50 dark:bg-[#0a0a0a]">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-5">
          ...
          <h1 className="text-4xl md:text-3xl font-bold text-gray-900 dark:text-white mb-4">
            Request <span className="text-[#B91C3C]">Blood</span>
          </h1>
          <p className="text-gray-600 dark:text-gray-400 text-lg">
            Submit an emergency blood request to reach potential donors quickly
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 mb-8">
          <button
            ...
            className={`flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-medium transition-colors ${
              activeTab === 'list'
                ? 'bg-[#B91C3C] text-white shadow-lg shadow-[#B91C3C]/25'
                : 'bg-white dark:bg-[#111111] text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-white/10 hover:border-[#B91C3C]/50'
            }`}
          >
          ...
          <div className="bg-white dark:bg-[#111111] border border-gray-200 dark:border-white/5 rounded-2xl p-6 md:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                ...
                className="w-full bg-[#B91C3C] hover:bg-[#B91C3C] text-white px-6 py-4 rounded-lg font-bold transition-colors flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
            ...
            {loadingRequests ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {requests.map((req, index) => {
                  ...
                  const isCreator =
                    currentUser.id === req.createdBy?._id ||
                    currentUser._id === req.createdBy?._id;
                  ...
                  return (
                    <div
                      ...
                      className={`border rounded-2xl p-6 flex flex-col transition-all duration-700 ${
                        isHighlighted
                          ? 'ring-4 ring-[#B91C3C] shadow-xl shadow-[#B91C3C]/20 scale-[1.02]'
                          : req.status === 'Accepted'
                            ? 'bg-green-50/50 dark:bg-green-900/10 border-green-200 dark:border-green-500/20'
                            : 'bg-white dark:bg-[#111111] border-gray-200 dark:border-white/5 hover:border-[#B91C3C]/30'
                      }`}
                    >
                      ...
                      {req.status === 'Accepted' ? (
                        <div className="flex flex-col gap-3 mt-auto">
                          ...
                          {currentUser &&
                          (isCreator ||
                            currentUser.id === req.acceptedBy?._id ||
                            currentUser._id === req.acceptedBy?._id ||
                            currentUser.id === req.acceptedBy ||
                            currentUser._id === req.acceptedBy)
                            ? (() => {
                                // The creator contacts the donor's phone, the donor contacts the requester's requested number.
                                const contactPhone = isCreator
                                  ? req.acceptedBy?.phone
                                  : req.contactNumber;
                                const contactLabel = isCreator
                                  ? 'Donor'
                                  : 'Requester';
                                ...
                                return (
                                  <div className="grid grid-cols-2 gap-3">
                                    <a
                                      ...
                                      className="w-full flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 dark:bg-white/5 dark:hover:bg-white/10 text-gray-900 dark:text-white py-2.5 rounded-lg font-medium transition-colors text-sm"
                                    >
                                    ...
                                    <a
                                      ...
                                      className="w-full flex items-center justify-center gap-2 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366] dark:bg-[#25D366]/20 dark:hover:bg-[#25D366]/30 py-2.5 rounded-lg font-medium transition-colors text-sm"
                                    >
                                      <svg
                                        className="w-4 h-4"
                                        fill="currentColor"
                        ...
                        <div className="mt-auto">
                          <div className="w-full flex items-center gap-2 mb-4">
                            ...
                            <button
                              ...
                              className="bg-red-50 hover:bg-[#B91C3C] text-[#B91C3C] hover:text-white dark:bg-[#B91C3C]/10 dark:hover:bg-[#B91C3C] p-2.5 rounded-lg transition-colors flex items-center justify-center shrink-0 border border-[#B91C3C]/20"
                            ...
                            req.donorResponses.length > 0 && (
                              <div className="border-t border-gray-100 dark:border-white/5 pt-4">
                                ...
                                <div className="space-y-3">
                                  {req.donorResponses.map(
                                    (resp: any, i: number) => (
                                      <div
                                        key={i}
                                        className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gray-50 dark:bg-black/20 p-3 rounded-xl text-sm border border-gray-100 dark:border-white/5"
                                      >
                                        <div>
                                          <p className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                                            {resp.donor?.name || 'Unknown'}
                                            <span className="bg-red-100 text-red-600 px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider">
                                              {resp.donor?.bloodGroup}
                                            </span>
                                          </p>
                                          <p className="text-gray-500 flex items-center gap-1 mt-1">
                                            <Phone className="w-3 h-3" />{' '}
                                            {resp.donor?.phone || 'Hidden'}
                                          </p>
                                        </div>
                                        {resp.status === 'Pending' &&
                                          req.status === 'Active' && (
                                            <div className="flex gap-2 shrink-0">
                                              <button
                                                ...
                                                className="text-green-700 font-bold px-3 py-1.5 bg-green-100 hover:bg-green-200 dark:bg-green-900/30 dark:hover:bg-green-900/50 rounded-lg transition-colors"
                                              >
                                                Accept
                                              </button>
                                              <button
                                                ...
                                                className="text-red-700 font-bold px-3 py-1.5 bg-red-100 hover:bg-red-200 dark:bg-red-900/30 dark:hover:bg-red-900/50 rounded-lg transition-colors"
                                              >
                                                Reject
                                              </button>
                                            </div>
                                        ...
                                        {resp.status === 'Accepted' && (
                                          <span className="text-green-600 font-bold px-2 py-1 bg-green-50 dark:bg-green-900/20 rounded-lg">
                                            Accepted
                                          </span>
                                        )}
                                        {resp.status === 'Rejected' && (
                                          <span className="text-[#B91C3C] font-medium text-xs px-2 py-1 bg-red-50 dark:bg-red-900/20 rounded-lg">
                                            Rejected
                                          </span>
                                        )}
                                      </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-auto">
                          <a
                            ...
                            className="w-full flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 dark:bg-white/5 dark:hover:bg-white/10 text-gray-900 dark:text-white py-2.5 rounded-lg font-medium transition-colors"
                          >
                            <Phone className="w-4 h-4" />
                            Call
                          </a>
                          {(() => {
                            const myResponse = req.donorResponses?.find(
                              (r: any) =>
                                r.donor?._id === currentUser.id ||
                                r.donor === currentUser.id ||
                                r.donor?._id === currentUser._id,
                            );

                            if (myResponse) {
                              if (myResponse.status === 'Pending') {
                                return (
                                  <div className="w-full text-center bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-500 py-2.5 rounded-lg font-medium border border-yellow-200 dark:border-yellow-900/30 text-sm flex items-center justify-center">
                                    Response Pending
                                  </div>
                                );
                              }
                              if (myResponse.status === 'Accepted') {
                                return (
                                  <div className="w-full text-center bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-500 py-2.5 rounded-lg font-medium border border-green-200 dark:border-green-900/30 text-sm flex items-center justify-center">
                                    You were Accepted!
                                  </div>
                                );
                              }
                              if (myResponse.status === 'Rejected') {
                                return (
                                  <div className="w-full text-center bg-gray-50 dark:bg-white/5 text-gray-500 py-2.5 rounded-lg font-medium border border-gray-200 dark:border-white/10 text-sm flex items-center justify-center">
                                    Offer Rejected
                                  </div>
                                );
                              }
                            ...
                            const isCurrentUserEligible =
                              currentUser && checkEligibilityLocal(currentUser);
                            ...
                            return (
                              <button
                                ...
                                className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg font-medium transition-colors ${isCurrentUserEligible ? 'bg-[#B91C3C] hover:bg-[#B91C3C] text-white' : 'bg-gray-300 dark:bg-gray-800 text-gray-500 cursor-not-allowed'}`}
                              >
          ...
          activeTab === 'received' && (
            <div className="w-full space-y-6">
              {loadingDirect ? (
                ...
                <div className="bg-white dark:bg-[#111111] border border-gray-200 dark:border-white/5 rounded-2xl p-12 text-center text-gray-600 dark:text-gray-400">
                  No direct requests received yet. Wait for people to find you
                  via Find Donors.
                </div>
              ) : (
                receivedRequests.map((req, idx) => (
                  <div
                    key={req._id || idx}
                    className="bg-white dark:bg-[#111111] border border-gray-200 dark:border-white/5 rounded-2xl p-6 sm:p-8 relative"
                  >
                    ...
                    <div className="space-y-3 mb-6 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-100 dark:border-white/5">
                      <p className="text-gray-700 dark:text-gray-300">
                        <span className="font-semibold">
                          Requester Blood Group:
                        </span>{' '}
                        {req.requester?.bloodGroup || 'N/A'}
                      </p>
                      <p className="text-gray-700 dark:text-gray-300">
                        <span className="font-semibold">
                      ...
                      {req.message && (
                        <p className="text-gray-700 dark:text-gray-300 mt-2 italic border-t border-gray-200 dark:border-white/10 pt-2 text-sm text-gray-500">
                          "{req.message}"
                        </p>
                      )}
                    </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-white/5 flex flex-wrap gap-4 justify-end">
                        <button
                          onClick={async () => {
                            try {
                              const response = await fetch(
                                `${import.meta.env.VITE_API_URL || ''}/api/direct-requests/${req._id}/status`,
                                {
                                  method: 'PUT',
                                  headers: {
                                    'Content-Type': 'application/json',
                        ...
                        <button
                          ...
                          className="px-6 py-2 rounded-xl bg-[#B91C3C] text-white font-medium hover:bg-[#B91C3C] transition-colors shadow-lg shadow-[#B91C3C]/25"
                        >
                          Accept
                        </button>
                      </div>
