...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button
              ...
              className="px-3 py-1.5 text-sm font-medium text-white bg-[#B91C3C] hover:bg-[#B91C3C] rounded-lg transition-colors"
            >
              Delete
            </button>
          </div>
  ...
  const handleUpdatePriority = async (id: string, newPriority: string) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || ''}/api/requests/${id}/priority`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <div className="pt-24 pb-20 min-h-screen bg-gray-50 dark:bg-[#050505] text-gray-900 dark:text-white selection:bg-[#B91C3C]/30 font-sans">
      {/* Background Glows */}
      <div className="fixed top-0 left-1/4 w-96 h-96 bg-[#B91C3C]/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-gradient-to-br from-[#B91C3C]/20 to-red-600/5 rounded-2xl flex items-center justify-center border border-[#B91C3C]/20 shadow-[0_0_30px_rgba(239,68,68,0.15)]">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">
                Here's what's happening with BloodLink today.
              </p>
            </div>
          </div>
          {onLogout && (
            <button
              onClick={onLogout}
              className="flex items-center gap-2 bg-black/5 dark:bg-white/5 hover:bg-[#B91C3C]/10 text-gray-700 dark:text-gray-300 hover:text-red-400 border border-gray-300 dark:border-white/10 hover:border-[#B91C3C]/30 px-5 py-2.5 rounded-xl font-medium transition-all w-fit group"
            >
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center min-h-[50vh] animate-in fade-in duration-500">
            <div className="w-10 h-10 border-4 border-[#B91C3C]/20 border-t-[#B91C3C] rounded-full animate-spin mb-4" />
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
              {stats.map((stat, index) => (
                ...
                >
                  <div
                    className={`absolute top-0 right-0 w-32 h-32 ${stat.bg} rounded-full blur-3xl -mr-10 -mt-10 transition-opacity opacity-50 group-hover:opacity-100`}
                  />
                  ...
                  <div className="flex justify-between items-start relative z-10">
                    ...
                    <div
                      className={`w-12 h-12 ${stat.bg} ${stat.border} border rounded-xl flex items-center justify-center`}
                    >
                  ...
                  <div className="mt-4 flex items-center gap-2 relative z-10">
                    <span
                      className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-md ${stat.trendUp ? 'bg-green-500/10 text-green-400' : 'bg-[#B91C3C]/10 text-red-400'}`}
                    >
                      {stat.trendUp ? (
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {activeTab === 'analytics' && (
                  <AnalyticsTab users={users} requests={requests} />
                )}
                {activeTab === 'users' && (
                  <UsersTab
                    users={users}
...
function AnalyticsTab({ users, requests }: { users: any[]; requests: any[] }) {
  ...
  const monthlyData = useMemo(() => {
    ...
    users.forEach(u => {
      ...
      const match = past6Months.find(
        m =>
          m.month === months[date.getMonth()] && m.year === date.getFullYear(),
      );
      if (match) {
        match.users++;
      }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-[#111111]/80 backdrop-blur-xl border border-gray-200 dark:border-white/5 rounded-2xl p-6 min-h-[380px] flex flex-col shadow-xl">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <Droplet className="w-5 h-5 text-[#B91C3C]" /> Blood Group
              Distribution
            </h3>
        ...
        <div className="bg-white dark:bg-[#111111]/80 backdrop-blur-xl border border-gray-200 dark:border-white/5 rounded-2xl p-6 min-h-[380px] flex flex-col shadow-xl">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <Activity className="w-5 h-5 text-[#B91C3C]" /> Donors by Division
            </h3>
            <button className="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
              <MoreVertical className="w-5 h-5" />
            </button>
      ...
      <div className="bg-white dark:bg-[#111111]/80 backdrop-blur-xl border border-gray-200 dark:border-white/5 rounded-2xl p-6 min-h-[400px] flex flex-col shadow-xl">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <BarChart3 className="w-5 h-5 text-[#B91C3C]" /> Monthly
            Registrations
          </h3>
          <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            <span className="flex items-center gap-1">
              <div className="w-2 h-2 rounded-full bg-[#B91C3C]"></div> Donors
            </span>
          </div>
        </div>
          ...
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
              ...
              <Line
                ...
                stroke="#ef4444"
                strokeWidth={3}
                dot={{ r: 4, fill: '#ef4444', strokeWidth: 2, stroke: '#111' }}
                activeDot={{
                  r: 6,
                  fill: '#ef4444',
                  stroke: '#fff',
      ...
      <div className="bg-white dark:bg-[#111111]/80 backdrop-blur-xl border border-gray-200 dark:border-white/5 rounded-2xl p-6 shadow-xl">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <Activity className="w-5 h-5 text-[#B91C3C]" /> Recent Activities
          </h3>
        ...
        <div className="space-y-4">
          {recentActivities.map(activity => (
            <div
              key={activity.id}
              className="flex items-start gap-4 p-4 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition-colors border border-transparent hover:border-gray-200 dark:border-white/5"
            >
              <div
                ...
                <p className="text-sm text-gray-900 dark:text-white">
                  {activity.type === 'registration' && (
                    <>
                      ...
                      registered as a new donor.
                    </>
                  )}
                  {activity.type === 'request' && (
                    <>
                      New blood request for{' '}
                      <span className="font-medium">{activity.user}</span>.
                    </>
                  )}
                  {activity.type === 'fulfilled' && (
                    <>
                      Blood request for{' '}
                      <span className="font-medium">{activity.user}</span> was
                      fulfilled.
                    </>
                  )}
                </p>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Manage all registered donors and admins.
          </p>
        </div>
        <div className="relative w-full sm:w-auto">
          ...
          <input
            ...
            className="w-full sm:w-72 bg-gray-100 dark:bg-[#1a1a1a] border border-gray-300 dark:border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#B91C3C] focus:ring-1 focus:ring-[#B91C3C] transition-all"
          />
        </div>
      </div>
          ...
          <tbody className="divide-y divide-white/5">
            {filteredUsers.length > 0 ? (
              filteredUsers.map(user => (
                <tr
                  key={user.id}
                  className="hover:bg-black/[0.03] dark:bg-white/[0.03] transition-colors group"
                >
                  ...
                  <td className="py-4 px-4">
                    <span className="inline-flex items-center justify-center w-8 h-8 rounded-full border border-[#B91C3C]/30 text-[#B91C3C] text-xs font-bold bg-[#B91C3C]/10 shadow-[0_0_10px_rgba(239,68,68,0.1)]">
                      {user.blood}
                    </span>
                  </td>
                  <td className="py-4 px-4">
                    <span
                      className={`inline-flex px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${user.role === 'Admin' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' : 'bg-blue-500/10 text-blue-400 border border-blue-500/20'}`}
                    >
                      {user.role}
                    </span>
                  </td>
    ...
    <div className="bg-white dark:bg-[#111111]/80 backdrop-blur-xl border border-gray-200 dark:border-white/5 rounded-2xl p-6 shadow-xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Monitor and manage emergency blood requests.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
          <div className="relative w-full sm:w-auto">
            ...
            <input
              ...
              value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
              className="w-full sm:w-64 bg-gray-100 dark:bg-[#1a1a1a] border border-gray-300 dark:border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#B91C3C] focus:ring-1 focus:ring-[#B91C3C] transition-all"
            />
          </div>
          <select
            value={filter}
            onChange={e => setFilter(e.target.value)}
            className="w-full sm:w-48 bg-gray-100 dark:bg-[#1a1a1a] border border-gray-300 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#B91C3C] focus:ring-1 focus:ring-[#B91C3C] transition-all appearance-none cursor-pointer"
          >
            ...
            {filteredRequests.length > 0 ? (
              filteredRequests.map(req => (
                ...
                >
                  ...
                  <td className="py-4 px-4">
                    <select
                      ...
                      className={`inline-flex px-2 py-1 rounded text-[11px] font-bold uppercase tracking-wider border focus:outline-none cursor-pointer ${
                        req.priority === 'High'
                          ? 'bg-[#B91C3C]/10 text-[#B91C3C] border-[#B91C3C]/20'
                          : req.priority === 'Medium'
                    ...
                    >
                      <option
                        value="Low"
                        className="bg-gray-100 dark:bg-[#1a1a1a] text-blue-500"
                      >
                        Low
                      </option>
                      <option
                        value="Medium"
                        className="bg-gray-100 dark:bg-[#1a1a1a] text-orange-500"
                      >
                        Medium
                      </option>
                      <option
                        value="High"
                        className="bg-gray-100 dark:bg-[#1a1a1a] text-[#B91C3C]"
                      >
                        High
                      </option>
                    </select>
                  ...
                  <td className="py-4 px-4">
                    <span
                      className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${req.status === 'Active' ? 'bg-yellow-500/10 text-yellow-500 border border-yellow-500/20' : 'bg-green-500/10 text-green-500 border border-green-500/20'}`}
                    >
                      {req.status}
                    </span>
                  </td>
                  <td className="py-4 px-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className={`p-2 rounded-lg transition-colors ${req.status === 'Active' ? 'text-green-400 hover:bg-green-500/10' : 'text-yellow-400 hover:bg-yellow-500/10'}`}
                        title={
                          req.status === 'Active'
                            ? 'Mark as Fulfilled'
                            : 'Mark as Active'
